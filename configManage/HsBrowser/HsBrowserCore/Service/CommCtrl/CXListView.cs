...
namespace HsServiceCore.Service.CommCtrl
{
    public partial class CXListView : ListView
    {
        ...
        protected override void OnDrawColumnHeader(DrawListViewColumnHeaderEventArgs e)
        {
            ...
            // e.DrawDefault = true;
            //e.Bounds
            //Rectangle headerRect = new Rectangle(e.Bounds.Location,this.Size);
            e.Graphics.FillRectangle(new SolidBrush(Color.FromArgb(207, 207, 207)), e.Bounds);

            StringFormat stringFormat = new StringFormat();
            stringFormat.LineAlignment = StringAlignment.Center;
            stringFormat.Alignment = StringAlignment.Center;

            e.Graphics.DrawString(this.Columns[e.ColumnIndex].Text, new Font("宋体", 10), new
                         SolidBrush(Color.FromArgb(78, 78, 78)), e.Bounds, stringFormat);

            e.Graphics.DrawLine(new Pen(Color.FromArgb(180, 180, 180)), (float)(e.Bounds.Right - 0.5),
                (float)0, (float)(e.Bounds.Right - 0.5), (float)e.Bounds.Bottom);
            e.Graphics.Dispose();
