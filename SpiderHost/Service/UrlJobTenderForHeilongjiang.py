...
dataCollection={}
IDLE_WAIT = 60*10
class UrlJobTenderForHebei(object):
    StopFlag = False
    def __init__(self):
        return

    def doWork(self,config):
        while not UrlJobTenderForHebei.StopFlag :
        ...
        while True:
            ...
            for index in range(len(oneUrl.Attrs)):
                ...
                if len(oneAttr.datas) <= rowIndex:
                    ...
                    if oneAttr.AttrName.lower() == "url":
                        shortUrl = "/zfcg/1/bidingAnncDetail_%s.html"

                        # 'watchContent(\\'104036\\',\\'1\\');'
                        valueTemp = valueTemp.replace("watchContent(","")
                        valueTemp = valueTemp.replace("'1');", "")
                        valueTemp = valueTemp.replace("'", "")
                        valueTemp = valueTemp.replace(",", "")
                        valueTemp = oneUrl.BaseUrl + shortUrl % valueTemp
