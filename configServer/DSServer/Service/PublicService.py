...
class PublicService(object):
    ...
    @staticmethod
    def checkUserPriv(userCode,privCode):
        return True

    @staticmethod
    def removeLocalFile(sype,ocode,Code,HashCode):
        filePath = os.path.join(HsShareData.AttchRoot, ocode)

        if sype == 0:
            filePath = os.path.join(filePath, "Product")

        filePath = os.path.join(filePath, Code)

        fileName = os.path.join(filePath, HashCode)
    ...
    @staticmethod
    def removeProduct(productCode,ocode):
        filePath = os.path.join(HsShareData.AttchRoot, ocode)
        filePath = os.path.join(filePath, "Product")
        filePath = os.path.join(filePath, productCode)
